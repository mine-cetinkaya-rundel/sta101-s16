...
sample of `r nrow(nycflights)` domestic flights that departed from the three New 
York City airport in 2013. We will generate simple graphical and numerical 
...
- Or we might want to determine which of the three NYC airports has a better 
...
The `dplyr` package offers seven verbs (functions) for basic of data 
manipulation:
...
This functions says to plot `dep_delay` from the `nycflights` data frame on the 
x-axis. It also defines a `geom` (short for geometric object), which describes 
the type of plot you will produce. 
...
save as a new data frame called `rdu_flights`.
    + `==` means "if it's equal to".
    + `RDU` goes in quotation marks because it's a character string.
- Line 2: Basically the same `qplot` call from earlier for making a histogram, 
except the data frame is not the data frame of all flights, but the newly 
constructed `rdu_flights`.
...
```{r}
rdu_flights %>%
  summarise(mean_ddel = mean(dep_delay), sd_ddel = sd(dep_delay), n = n())
```

Note that in the `summarise` function we created a list of two elements. The 
names of these elements are user defined, like `mean_ddel`, `sd_ddel`, `n`, and 
...
1.  Describe the distribution of the arrival delays of these flights using a 
    ...
    statistics you need will depend on the shape of the distribution.
...
Here, we first grouped the data by gender, and then calculated the summary 
statistics.
...
We can also the distributions of departure delays across months using 
...
months on the x-axis to produce side-by-side box plots. Then what does `factor()`
stand for? R calls categorical variables `factor`s. Months are coded as numbers
1-12 in the data frame, and hence R automatically assumes `month` to be a 
numerical variable. `factor(month)` forces the month variable to be treated as 
categorical.

### On time departure rate for NYC airports

Suppose you will be flying out of NYC and want to know which of the 
three airports has the best on time departure rate of departing flights. 
...
we want to create, `dep_type`. Then if `dep_delay < 5` we classify the flight
as `"on time"` and `"delayed"` if not (i.e. if the flight is delayed for 5 
minutes or more).
...
-   Mutate the data so that it includes a new variable that contains the 
